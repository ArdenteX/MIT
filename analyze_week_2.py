...
class PlotMap:
    ...
    def __init__(self):
        ...
        self.OUTPUT_PATH = 'D:\\广商网课\\resource\\Covid-19\\'
        self._OUTPUT_PATH_AFTER = 'D:\\广商网课\\resource\\Covid-19\\after\\'

    def convert_data(self):
        files = os.listdir(self._OUTPUT_PATH_AFTER)[1:]
        for i in range(len(files)):
            files[i] = self._OUTPUT_PATH_AFTER + files[i]
        count_dict = {}
...
class PlotPie:
    ...
    def convert_data(self):
        ...
        for file in files:
            ...
            if 'Missing' in data.index:
                data.drop(['Missing'], axis=0, inplace=True)

            elif 'NA' in data.index:
                data.drop(['NA'], axis=0, inplace=True)

            elif 'Unknown' in data.index:
                data.drop(['Unknown'], axis=0, inplace=True)

            tmp = data.to_dict()['age_group']
            _merge_and_plus(count_dict, tmp)
